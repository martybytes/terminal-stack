...
# in between, and switching channel. Driven by `ts-config wezterm` and runnable
# standalone.
#
# WezTerm ships two channels and this stack installs NEITHER automatically. The
# wizard asks at install, ts-update offers when something newer exists, and this
...
HELP='ts-wezterm — WezTerm build info, upstream comparison, and channel switching.

Usage:
  ts-wezterm [status]        installed build + date, latest per channel, what changed
  ts-wezterm changes         the full upstream changelog since your build (paged)
  ts-wezterm install <chan>  stable | nightly — switches channel, removing the other
  ts-wezterm upgrade         refresh the channel you are already on; never switches
  ts-wezterm -h              this help
...
# Help before anything else: `ts-wezterm -h` must work on a box where the clone
...
if [ -z "$CZ" ]; then
    ...
    else echo "ts-wezterm: chezmoi not found on PATH." >&2; exit 1; fi
fi
SRC="${TERMINAL_STACK_DIR:-$("$CZ" source-path 2>/dev/null || true)}"
if [ ! -d "$SRC/bootstrap" ]; then
    echo "ts-wezterm: cannot locate the terminal-stack clone (set TERMINAL_STACK_DIR)." >&2
    exit 1
fi
...
show_changes() {
    ...
    ver="${inst%%|*}"
    if [ -z "$ver" ]; then
        echo "ts-wezterm: WezTerm is not installed, so there is no build to compare against." >&2
        return 1
    fi
    if ! text="$(ts_wez_changes_text "$ver")"; then
        echo "ts-wezterm: could not fetch upstream's changelog (offline?)." >&2
        return 1
    fi
    if [ -z "$text" ]; then
...
case "${1:-}" in
    ...
    install)
        case "${2:-}" in
            ...
            *) echo "usage: ts-wezterm install <stable|nightly>" >&2; exit 2 ;;
        esac ;;
    upgrade)   ts_wezterm_upgrade ;;
    *) echo "ts-wezterm: unknown command '$1' (try: status, changes, install, upgrade, -h)" >&2; exit 2 ;;
esac
