...
ts_doctor() {
    ...
    if [ "$(ts_agent_get agentmemoryEnabled 2>/dev/null || echo off)" = on ] && [ -n "${src:-}" ]; then
        ...
        if [ "$_am_done" = 0 ] && [ -f "$src/bootstrap/ts-agentmemory.sh" ]; then
            ...
            # returns 0 when there is none, so a machine with no agentmemory plugin at
            # all reported `ok wiring intact` while capturing nothing. Ask the same
            # question --check asks -- does a plugin cache exist -- before believing it.
            if [ -n "$(ls -1 "$HOME/.claude/plugins/cache/agentmemory/agentmemory" \
                              "${CODEX_HOME:-$HOME/.codex}/plugins/cache/agentmemory/agentmemory" \
                ...
                _bad "agentmemory is enabled but its plugin is not installed for any agent — nothing captures: ts-config agents agentmemory repair"
            fi
        fi
        ...
        #
        # EVERY substitution below ends in `|| true`, and that is load-bearing
        # rather than decorative. ts-doctor runs under `set -euo pipefail`, so
        # each of these three lines used to ABORT the whole run before printing a
        # summary: `docker ps | grep -v console` exits 1 when no container
        # matches, `docker exec` exits non-zero when it fails, and `cmd.exe`
        # exits 127 on every host that is not Windows. Only `ts_repair` survived
        # it, and only because it wraps ts_doctor in `|| true`.
        local _am_cid _am_csec _am_usec
        if command -v docker >/dev/null 2>&1; then
            _am_cid="$(ts_timeout 5 docker ps --filter name=agentmemory --format '{{.Names}}' 2>/dev/null \
                | grep -v console | head -1 || true)"
            if [ -n "$_am_cid" ]; then
                _am_csec="$(ts_timeout 5 docker exec "$_am_cid" cat /data/.hmac 2>/dev/null | tr -d '\r\n' || true)"
                _am_usec=""
                if [ -d /mnt/c/Users ]; then
                    ...
                    _am_usec="$(ts_timeout 5 cmd.exe /c 'echo %AGENTMEMORY_SECRET%' 2>/dev/null | tr -d '\r\n' || true)"
                    case "$_am_usec" in *'%AGENTMEMORY_SECRET%'*) _am_usec="" ;; esac
                elif [ -f "$src/bootstrap/_agentmemory.sh" ]; then
                    # Unix has no HKCU\Environment equivalent, so the 0600 cache IS
                    # what a hook recovers to. Which paths those are belongs to
                    # _agentmemory.sh, which also owns the injected reader that
                    # loops them; sourced in a subshell so nothing leaks in here.
                    # Whole group redirected, not just the source: during an update the
                    # clone on disk can be older than this file, and a bare call would
                    # spray "am_secret_cache_read: command not found" across the report.
                    _am_usec="$( { . "$src/bootstrap/_agentmemory.sh" && am_secret_cache_read; } 2>/dev/null || true )"
                fi
                if [ -n "$_am_csec" ] && [ -n "$_am_usec" ] && [ "$_am_csec" != "$_am_usec" ]; then
                    _bad "agentmemory secret mismatch: the container's differs from the one your hooks would recover — every request 401s and is swallowed; refresh it with the plugin's setup"
                fi
            fi
        fi
    fi
