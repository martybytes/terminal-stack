...
- **PowerShell 7 `$PROFILE`** with Starship prompt, OSC 7 cwd hint, tilde-abbreviated tab title, UTF-8 console restore (heals Claude-Code `Γ¥»` mojibake), Claude wrappers that set per-tab project titles, and enhanced `cy` / `cyr` Codex yolo wrappers.
- **WSL zsh** with oh-my-zsh, theme cleared so Starship owns the prompt, a `precmd` that sets tab titles, `ccs` / `ssht` helpers for tmux-attached Claude Code and SSH sessions, and the same enhanced Codex wrappers.
- **Three-line Codex dashboard + TTS** for `cy` (`codex --yolo`) and `cyr` (`codex --yolo resume`): project/Git/repo, model/effort/context and 5-hour/weekly usage bars, then identity/tokens/session patch/permissions/version. It occupies a three-row WezTerm split beneath Codex and disappears with the session; ordinary `codex` keeps its native footer. See `doc codex`.
