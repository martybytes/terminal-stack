...
**Claude/Cursor voice notifications (`ccTts*`).** Opt-in TTS (`ccTtsEnabled`, default off) speaks when Claude Code / Cursor finishes, errors, or needs input — engines Kokoro (Docker :8880) → Chatterbox → edge-tts, config from `~/.claude/tts/config.json` + untracked `local.json`, module `bootstrap/_cc_tts.sh` + pwsh twins in `_config.ps1`, `ts-config tts …` both shells. Layered on top: the **`ttsd` tray daemon** (`ccTtsDaemon`, default off; source `bootstrap/tts-daemon/ttsd`, chezmoi-ignored, venv at `%LOCALAPPDATA%\terminal-stack\tts-daemon\venv` outside the clone) — session-naming, queue/coalesce, music duck/pause, four summarizer modes (`self` installs a marker block in the user's `~/.claude/CLAUDE.md` — never whole-file-manage that file). Hooks POST to `127.0.0.1:8890` and **must keep the direct cc-tts-notify fallback — never silence**; routing is decided inside the hook scripts, so the managed `settings.json`/`hooks.json` templates and both sync renderers carry **no daemon state** (don't add tokens for it). The daemon is never auto-restarted (`ts-update` nudges; `ts-config tts daemon restart` is the deliberate path — same rule as the mux server), and the duck engine must keep its snapshot-before-change discipline (Windows persists per-app volume). On a combined Windows+WSL machine, `ts-config tts …` must run **from WSL** to stick — pwsh saves only the Windows `config.json`, which the next WSL apply re-renders from `[data]`. And any pwsh function that runs a child process *and* returns a status must stream via `| Out-Host` — bare `& pwsh` captures the child's stdout into the return value and turns failures into truthy arrays (`docs/powershell-quirks.md` § "`& child.exe` inside a function"). Rationale entries: `docs/decisions.md` §§ TTS daemon.
