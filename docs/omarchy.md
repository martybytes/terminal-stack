...
## `~/.claude/settings.json`, which three things write

1. On this fleet it is a **symlink** into omarchy-dots' stow tree.
2. `omarchy-theme-set-claude --activate` writes `.theme` (`custom:omarchy`) and
   keeps `~/.claude/themes/omarchy.json` in step, which Claude Code hot-reloads.
3. The stack's chezmoi `modify_` script splices `statusLine`, `hooks` and — off
   Omarchy — `theme`.

Two changes make those coexist.

**The stack cedes `theme` on Omarchy**, and only there. Ownership in that splice
is per key and derived from what the fragment renders, so dropping the key *is*
the mechanism: the script only overwrites keys the fragment produces. Otherwise
the flat light/dark token and `custom:omarchy` alternate on every apply and every
theme change, with nothing to explain the flapping.

**The write goes through the symlink.** This was measured, not assumed — on
archlinux with chezmoi 2.72, a symlinked target and a one-line `modify_` script:
...
The splice succeeded, the link was gone, and the file omarchy-dots tracks was
left behind — still referenced by its repo, now permanently stale, silently. The
`modify_` script cannot fix that (it only produces bytes; *chezmoi* does the
write), so a pair of apply hooks does:
`run_before_25-claude-settings-link-record.sh` records the link's destination and
`run_after_25-claude-settings-link-restore.sh` copies the spliced content to it
...
`ufw` is active here and `ufw-docker` is installed, because Docker writes its
own iptables rules and a published port bypasses ufw entirely — a container on
`0.0.0.0:8788` is LAN-reachable while `ufw status` says it is blocked.

**That is already handled, and it was already handled before Omarchy.** Every
published port in every stack binds `127.0.0.1` explicitly, and
`tests/test_stack.py::test_every_published_port_binds_loopback_only` globs every
`docker-compose*.yml` under `services/` and fails on any that does not;
`tests/test_services.py` audits the same thing against the running engine. None
of these services authenticates, so the binding is the only thing standing
between them and the network — which is why it is a gate and not a convention.
