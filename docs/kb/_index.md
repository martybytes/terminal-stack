...
- `common/tools/` — per-tool cheat-sheets (eza, fzf, bat, fd, tree, ripgrep, zoxide, atuin, herdr, yazi, delta, starship, chezmoi, micro, glow, neovim, zed, cursor, wezterm, ghostty, grok, gemini, pi, llmfit, node-python, tailscale, rclone, and disk/network monitors) — `doc eza`, `doc tailscale`, …
...
- `linux/` — apt, VeraCrypt, ssh permissions, systemd, docker (engine install, rootless without the docker group, NVIDIA toolkit), Omarchy (`doc omarchy`: pacman, the bash-first login shell, the XDG tmux path)
