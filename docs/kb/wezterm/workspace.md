...
Leader: **`Ctrl+\`** — tap, release, then press the next key. It **waits** (no
timeout); `Ctrl+\` `Esc` cancels.
...
| Key | Action |
|---|---|
| `Ctrl+\` `w` | switch — fuzzy picker of existing workspaces |
| `Ctrl+\` `n` | new / switch — type a name to create or jump to one |
| `Ctrl+\` `p` | **project picker** (sessionizer) — fuzzy-pick a repo from the `wso` workspace tree (and `*_Personal`/`*_Public`/`*_Work` siblings), switch to a workspace for it. Needs `fd` on PATH |

## Manage
| Key | Action |
|---|---|
| `Ctrl+\` `R` | rename the current workspace |
| `Ctrl+\` `X` | kill — close every pane in this workspace |
...
autosave keeps running, so `Ctrl+\` `L` restores a session by hand whenever
...
| Key | Action |
|---|---|
| `Ctrl+\` `S` | save the current workspace now |
| `Ctrl+\` `L` | restore a saved state — fuzzy picker |
...
**`Ctrl+\` `s`** adds `user@host │ path` for the **active pane** (the kind of
