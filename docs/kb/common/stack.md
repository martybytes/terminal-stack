...
| `tstack update` | pull the latest stack and re-apply — see below |
| `tstack rollback` | undo the last `tstack update` — see below |
...
you deliberately work against; that pin is how `tstack update` can update it, since dev
clones are otherwise skipped. At a legacy path `tstack update` prints a one-line notice
and `tstack doctor --repair` offers to move the clone to the canonical location (git
state intact).
...
`tstack update`, `wso` and `doc` down together. An explicit `sync-windows.ps1 -SourceDir`
...
Before applying, `tstack update` checks chezmoi for files changed both locally and
...
The runtime clone must be clean. If it is dirty, `tstack update` stops before fetch,
pull, or apply and lists the files. Make the change in a workspace dev clone,
commit it, then rerun `tstack update`.
...
recorded rollback point" means no `tstack update` has pulled commits since the state
...
`tstack update` that pulls overwrites the recorded point.
...
Also reachable as `tstack config wezterm …`, and `t` in the `tstack config` menu.
...
Nothing here runs on its own: the wizard asks at install, `tstack update` offers when
...
| Command | What it does |
|---|---|
| `tstack mux` / `tstack mux status` | the setting, the *rendered* setting, the server pid, the pane count |
| `tstack mux on` / `tstack mux off` | flip it and re-render `.wezterm.lua` |
...
| `tstack mux restart` | stop it, start a fresh one, so it re-reads the config |
...
so a config change needs `tstack mux restart` (which kills every pane) and not just a
...
Panes already hosted by the mux stay there until you close them or `tstack mux kill`.
...
(`tstack config show` prints it as `wezmux`; `tstack config mux …` is the same command).
On WSL the mux server is a **Windows** process — `tstack mux` reaches it over interop,
...
| Command | What it does |
|---|---|
| `tstack smb setup` | guided Tailscale discovery, sign-in, verified share selection, and preview |
| `tstack smb` / `tstack smb list` | live mounts: name, state, engine, mountpoint (strays flagged) |
| `tstack smb hosts` | SMB servers advertising on this LAN over mDNS |
...
| `tstack smb probe … --write` | also test writability (creates a file; asks first) |
| `tstack smb ls` / `tree` / `du` | look inside without mounting (`--depth N` for tree) |
| `tstack smb get SRC DST` | copy out without mounting |
| `tstack smb add NAME` | add a share (`--host`/`--path`/`--user` to skip the prompts) |
| `tstack smb creds NAME set` | store the password, obscured, in the OS keychain |
| `tstack smb mount NAME` | mount read-only (`--rw`, `--at DIR`, `--engine auto\|fuse\|nfs`) |
| `tstack smb umount NAME` | unmount (`--all`, `--force`) |
...
Nothing has to be configured to interrogate a host: `tstack smb shares nas.lan` builds
...
wrong and `tstack smb doctor` says so by name.

Passwords are obscured once by `tstack smb creds` and kept in the OS keychain (macOS
...
If a mount will not work, `tstack smb doctor` is the command. On macOS it knows the
...
(`tstack smb` pins it, so this cannot bite), and FUSE-T's **FSKit** backend is not
