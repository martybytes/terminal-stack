...
| `tstack config leader <chord>` | WezTerm leader, e.g. `ctrl-backslash` (default), `ctrl-space`, `ctrl-a`, `alt-x` (keys with no printable spelling are named — the wizard accepts `ctrl-\` and spells it for you, this verb does not) |
