...
It runs the wizard (leader key / theme / WezTerm / mux / apps / TTS / workspace — see **Install wizard** above; set `$env:TS_LEADER`/`TS_THEME`/`TS_WEZTERM`/`TS_WEZ_MUX`/`TS_APPS` to skip prompts), shows the review, and only then installs:
